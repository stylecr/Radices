...
int map_clearflooritem_timer (int tid, unsigned int tick, int id, intptr_t data)
{
	...
	else if (fitem->item_data.card[0] == CARD0_PET) // pet egg
...
{
	puts ("Uso: map-server [opÁıes]");
	puts ("OpÁıes:");
	puts (CL_WHITE"  Comandos\t\t\tDescriÁ„o"CL_RESET);
	puts ("-----------------------------------------------------------------------------");
...
static void map_versionscreen (bool do_exit)
{
	...
	ShowInfo (CL_WHITE"Revis„o do Cronus: "CL_GREEN"%s" CL_RESET"\n", rev);
...
int do_init (int argc, char *argv[])
{
	...
	if (battle_config.pk_mode)
		ShowNotice ("O servidor est· sendo executado em "CL_WHITE"Modo PK"CL_RESET".\n");

	ShowStatus ("O map-server est· "CL_GREEN"pronto"CL_RESET" e funcionando pela porta "CL_WHITE"%d"CL_RESET".\n\n", map_port);
