...
DBMap *script_get_label_db()
{
	return scriptlabel_db;
}
DBMap *script_get_userfunc_db()
{
	return userfunc_db;
}
...
/*==========================================
 * ƒ[ƒJƒ‹ƒvƒƒgƒ^ƒCƒvéŒ¾ (•K—v‚È•¨‚Ì‚Ý)
 *------------------------------------------*/
...
static void add_scriptl (int l)
{
	...
	switch (str_data[l].type) {
		...
		case C_NOP:
		case C_USERFUNC:
			// ƒ‰ƒxƒ‹‚Ì‰Â”\«‚ª‚ ‚é‚Ì‚Åbackpatch—pƒf[ƒ^–„‚ßž‚Ý
			add_scriptc (C_NAME);
...
//	 if, switch, while ‚ð‚±‚Ì“à•”‚Åˆ—‚µ‚Ü‚·B
const char *parse_syntax (const char *p)
{
	...
	switch (*p) {
		...
		case 'F':
			if (p2 - p == 3 && !strncasecmp (p, "for", 3)) {
				...
				parse_line (label);
				syntax.curly_count--;
				// ŽŸ‚Ìƒ‹[ƒv‚Ö‚Ìƒ‰ƒxƒ‹Œ`¬‚·‚é
				sprintf (label, "__FR%x_NXT", syntax.curly[pos].index);
				l = add_str (label);
				set_label (l, script_pos, p);
				// ŽŸ‚Ìƒ‹[ƒv‚É“ü‚éŽž‚Ìˆ—
				// for ÅŒã‚Ì ')' ‚ð ';' ‚Æ‚µ‚Äˆµ‚¤ƒtƒ‰ƒO
	...
	*flag = 1;
	...
	if (syntax.curly_count <= 0) {
		*flag = 0;
		...
		if (syntax.curly[pos].flag) {
			// Œ»Ý’n‚Ìƒ‰ƒxƒ‹Œ`¬‚·‚é(continue ‚Å‚±‚±‚É—ˆ‚é)
...
void script_free_vars (struct linkdb_node **node)
{
	...
	while (n != NULL) {
		...
		if (is_string_variable (name))
			aFree (n->data); // •¶ŽšŒ^•Ï”‚È‚Ì‚ÅAƒf[ƒ^íœ
...
BUILDIN_FUNC (getscrate)
{
	...
	if (script_hasdata (st, 4)) //Žw’è‚µ‚½ƒLƒƒƒ‰‚Ì‘Ï«‚ðŒvŽZ‚·‚é
...
BUILDIN_FUNC (globalmes)
{
	...
	mes = script_getstr (st, 2);	// ƒƒbƒZ[ƒW‚ÌŽæ“¾

	if (mes == NULL) return 0;

	if (script_hasdata (st, 3)) {	// NPC–¼‚ÌŽæ“¾(123#456)
		name = script_getstr (st, 3);
	} else {
		name = nd->name;
	}
...
BUILDIN_FUNC (successremovecards)
{
	...
	for (c = sd->inventory_data[i]->slot - 1; c >= 0; --c) {
		if (sd->status.inventory[i].card[c] && itemdb_type (sd->status.inventory[i].card[c]) == IT_CARD) {
			...
			if ( (flag = pc_additem (sd, &item_tmp, 1))) {	// Ž‚Ä‚È‚¢‚È‚çƒhƒƒbƒv
				clif_additem (sd, 0, 0, flag);
